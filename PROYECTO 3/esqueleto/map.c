...
map_t map_put(map_t map, key_t key, value_t value)
{
	...
	else
	{
		...
		{
			aux->left=map_put(aux->left,key,value);
		}
		else if (string_less(aux->key,key))
		{
			aux->right=map_put(aux->right,key,value);
		}
	}
	return map;
}
...
bool map_contains(map_t map, key_t key)
{
	...
	else
	{
		if (string_eq(key,aux->key))
			...
			res=map_contains(aux->left,key);
		else 
			res=map_contains(aux->right,key);
	}
	return res;
}
