...
void swap (item_t a[], unsigned int pos1, unsigned int pos2) {
	item_t temp=NULL;
	temp=a[pos1];
	a[pos1]=a[pos2];
	a[pos2]=temp;
...
void insertion_sort(item_t a[], unsigned int length) {
	for(unsigned int i=1;i<length;i++)
	{
		unsigned int j=i;
		for(j=i;j>0 && item_value(a[j])>item_value(a[j-1]);j--)
		{
			swap(a,j-1,j);
		}
	}    
...
int run_file(const char *filename,
	...
	if (show_items) {
		...
		item_t *items_selected = calloc(size, sizeof(item_t));
		unsigned int j=0;
		for (unsigned int i = 0; i < array_length; i++) 
		{
			if (selected[i]) 
		...
		insertion_sort(items_selected,size);
		for(unsigned int i=0;i<size;i++)
		{
			item_t item = items_selected[i];
